...
function mc_cl_enqueue_scripts() {
  ...
  wp_register_script(
    'mc_googleapi', 
    "https://maps.googleapis.com/maps/api/js?libraries=drawing,geometry&key=$googleapi&callback=initMap", 
    array('mc_markercluster'), 
    '', 
    true 
  );
  wp_localize_script( 
...
function sqlrequest() {
  if ( isset($_POST['sqlstmt']) ) {
    ...
    if ( $sqlrequest['customers_active'] == 'true' ) {
      array_push(
        $whereclause, 
        '(Status IN ("Active", "Hibernating", "Delinquent"))'
      );
    }
    if ( $sqlrequest['customers_nonactive'] == 'true' ) {
      array_push(
        $whereclause, 
        'Status = "Not Active" OR Status = "Unistall" OR Status = "Collections"'
      );
    } 
    if ( $sqlrequest['leads_leads'] == 'true' ) {
      array_push(
        $whereclause, 
        'CancelReason = "" AND (Status = "Pre Install" OR Status = "Lead")'
      );
    } 
    ...
    if ( $sqlrequest['leads_wocancelled'] == 'true' ) {
      array_push(
        $whereclause, 
        '(CancelReason = "Cancelled" AND Status NOT IN ("Active", "Hibernating", "Delinquent"))'
      );
    } 
    if ( $sqlrequest['leads_womaxap'] == 'true' ) {
      array_push(
        $whereclause, 
        '(CancelReason = "Maxed AP" AND Status NOT IN ("Active", "Hibernating", "Delinquent"))'
      );
    } 
    if ( $sqlrequest['leads_wolosttocompetitor'] == 'true' ) {
      array_push(
        $whereclause, 
        '(CancelReason = "Lost to Competitor" AND Status NOT IN ("Active", "Hibernating", "Delinquent"))'
      );
    } 
    ...
    if  ( $key !== 'false' ) {
      $sqlkeyword = "
        id LIKE '%$key%' OR Status LIKE '%$key%' 
        OR CancelReason LIKE '%$key%' OR FirstName LIKE '%$key%' 
        OR LastName LIKE '%$key%' OR CompanyName LIKE '%$key%' OR 
        Phone LIKE '%$key%' OR Email LIKE '%$key%' 
        OR Address LIKE '%$key%' OR City LIKE '%$key%' 
        OR State LIKE '%$key%' OR Zip LIKE '%$key%'
      ";
    ...
    $customerresults = $wpdb->get_results( 
      "SELECT * FROM ".$table_prefix."customerlist WHERE ($whereclause) $state" 
    );
