	...
	if (document.querySelectorAll('.packages')) {
		...
		function packageTemplate(i, count, rev) {
			...
			if (i.querySelector('.content').childNodes.length > 0) {
				...
				show_con = `<div data-accordion="accordion" style="min-height:22px;">
					<button data-accordion="head" class="show-content">
					<span class="arr">&#10095; </span> ${details} </button>
					<div data-accordion="body" class="din-content"><hr>
					${i.querySelector('.content').innerHTML}</div></div>`;
			}
			i.insertAdjacentHTML("afterbegin", `   
				<div class="hover">
				<h2 class="amount">${x.price ? price : '&nbsp;'}</h2>
				<div class="anim-container"><div class="anim">${cell}</div><div class="hide"></div></div>
				<h2 class="title bg-${rev}">${x.title}</h2>
				${x.plan_details ? `<p>${x.plan_details}</p>` : ''}
				</div>
				<strong>${promo}</strong>
				<div class="container">
				<div class="contains">${show_con}</div>
				</div>
				<div class="action"> ${cta = x.cta ? x.cta :  'Add to Cart'} </div>
